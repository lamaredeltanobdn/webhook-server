...
app.get('/test-order', async (req, res) => {
  ...
  try {
    ...
    if (!tokenData.access_token) return res.status(500).json({ error: 'No access_token', details: tokenData });
    ...
    const simRes = await fetch('https://test-api.uber.com/v1/eats/sandbox/orders', {
      ...
      body: JSON.stringify({ store_id: process.env.UBER_STORE_UUID })
    });
...
app.get('/test-webhook', async (req, res) => {
  ...
  try {
    const fakeOrder = { event_type: 'orders.notification', meta: { order_id: 'TEST-' + Date.now(), store_id: process.env.UBER_STORE_UUID } };
    const { error } = await supabase.from('pedidos_ubereats').insert([{ order_id: fakeOrder.meta.order_id, store_id: fakeOrder.meta.store_id, status: 'nuevo', raw_data: fakeOrder, created_at: new Date().toISOString() }]);
    ...
    res.json({ status: 'ok', order_id: fakeOrder.meta.order_id });
  } catch (err) {
    console.error('==> ERROR:', err.message);
    res.status(500).json({ error: err.message });
  }
...
app.post('/webhook', async (req, res) => {
  try {
    ...
    if (event.event_type === 'orders.notification') {
      ...
      await supabase.from('pedidos_ubereats').insert([{ order_id: order.order_id, store_id: order.store_id, status: 'nuevo', raw_data: event, created_at: new Date().toISOString() }]);
    }
